...
/// to be using. Probably best to put them all here, not in AgentController.
/// </summary>
...
public class SteeringBehavior : MonoBehaviour {
    ...
    private Vector3 FlockingCoh() {
        ...
        int collCount = 0;
        foreach (Collider coll in cylnCheck.CollidersClose) {
            AgentController collNPC = coll.gameObject.GetComponent<AgentController>();
    ...
    public Vector3 FlockFollowPath() {
        ...
        if (Vector3.Distance(agentTrans.position, pathPoints[currentPathPointIndex].position) < targetRadiusL) {
            if (currentPathPointIndex < pathPoints.Count - 1) {
                // hard code
                /**
                if (GetComponent<AgentController>().followers.Count>0 && currentPathPointIndex == 3) {
                    GetComponent<AgentController>().SetFollowerTarget(pathPoints[currentPathPointIndex].gameObject);
                }
                if (GetComponent<AgentController>().followers.Count > 0 && currentPathPointIndex == 4) {
                    GetComponent<AgentController>().SetFollowerTarget(pathPoints[currentPathPointIndex].gameObject);
                }
                if (GetComponent<AgentController>().followers.Count > 0 && currentPathPointIndex == 5) {
                    GetComponent<AgentController>().SetFollowerTarget(pathPoints[currentPathPointIndex].gameObject);
                }
                if (GetComponent<AgentController>().followers.Count > 0 && currentPathPointIndex == 6) {
                    GetComponent<AgentController>().ClearFollowerTarget();
                }**/
